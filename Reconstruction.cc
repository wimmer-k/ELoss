...
double Reconstruction::StoppingPower(Nucleus* target, double energy, bool gaseous = false){
  int z_p = fProj->GetZ();
...
double Reconstruction::CompoundRange(double energy, int limit){
  ...
  if( (energy<en_1) || (z_p<1) || (a_p<1) )
    return 0;
  n = -limit - 1 + (int) floor(log(energy)*0.4342944819 + 0.5);
  ...
  for(int i=0;i<n;i++){
    h = (en_2 - en_1)/6.;
    range += h*(0.3/StoppingPower(en_1) + 1.5/StoppingPower(en_1 + h)
		+ 0.3/StoppingPower(en_1 + 2*h) + 1.8/StoppingPower(en_1 + 3*h)
		+ 0.3/StoppingPower(en_2 - 2*h) + 1.5/StoppingPower(en_2 - h)
		+ 0.3/StoppingPower(en_2));
    en_1 = en_2;
    en_2 *= 10;
  }
  return range;
...
double Reconstruction::EnergyAfter(double energy, int limit){ // return lost energy!
  ...
  if(fTargetThickness<0){
    cerr << "Target thickness not set!" << endl;
    return 0;
  }
  en = energy;
  x = 0;
  if(energy <= 0){
    ...
    range = CompoundRange(energy, limit);
    dedx = StoppingPower(energy);
  }
...
double Reconstruction::EnergyStraggling(double dE_dx, double dE_dx_after_target, double energy_loss){
  ...
  double a_p = fProj->GetA();
  int n_el = fTarget->GetNofElements();
  ...
  for(int i=0;i<n_el;i++){
    z_t += fTarget->GetNucleus(i)->GetZ()*fTarget->GetFrac(i);
  }
...
double Reconstruction::AngularStraggling(double energy){
  ...
  for(int i=0;i<n_el;i++){
    z_eff += sqrt(pow(z_p, 2./3.)+pow(fTarget->GetNucleus(i)->GetZ(), 2./3.));
    tau = 41470.0*fTargetThickness/(pow(z_eff,2)*fTarget->GetNucleus(i)->GetA());
    exponent = pow(log(1.03+tau),-0.115) - 0.115;
    ang += fTarget->GetNucleus(i)->GetZ()*z_eff*pow(tau,exponent); // should be a += ??
  }
  if(energy < 1.e-30)
...
double Reconstruction::ChargeState(double energy){
...
void Reconstruction::Print(double energy){
  //double range = CompoundRange(energy, -7);
  //double dE_dx = StoppingPower(energy);
  //double en_after_target = EnergyAfter(energy,-7);
  //double en_loss = EnergyLoss(energy,-7);
  //double dE_dx_after_target = StoppingPower(en_after_target);
  //double en_straggling = EnergyStraggling(dE_dx,dE_dx_after_target,en_loss);
  //double ang_straggling = AngularStraggling(energy);
  //double chargestate = ChargeState(energy);
  double result[9];
  result[0] = energy;
  result[1] = CompoundRange(energy, -7);
  result[2] = StoppingPower(energy);
  result[3] = EnergyAfter(energy,-7);
  result[4] = EnergyLoss(energy,-7);
  result[5] = StoppingPower(result[3]);
  result[6] = EnergyStraggling(result[2],result[5],result[4]);
  result[7] = AngularStraggling(energy);
  result[8] = ChargeState(energy);
  for(int i=0;i<9;i++)
    cout << result[i] << "\t";
  cout << endl;
  return;
...
void Reconstruction::Print(double from, double to, int steps){
  cout << "energy\trange\tdE_dx\ten_after\ten_loss\tdE_dx_after\ten_strag\tang_strag\tchargestate" << endl;
  if(steps == 1){
    Print(from);
    return;
  }
  for(int i=0;i<steps;i++){
    Print(from+i*(to-from)/(steps-1));
  }
  return;
}
TSpline3* Reconstruction::Energy2Range(double emax, double size){
  double* range = new double[(int)(emax/size)+1];
  double* energy = new double[(int)(emax/size)+1];
  
  for(int i=0;i<(emax/size);i++){
    energy[i] = i*size;
    range[i] = CompoundRange(energy[i],-5);
...
TSpline3* Reconstruction::Energy2EnergyLoss(double emax, double size){
  ...
  for(int i=0;i<(emax/size);i++){
    energy[i] = i*size;
    eloss[i] = EnergyLoss(energy[i],-5);//*1000.; //conversion to keV
    //energy[i]*=1000.; //conversion to keV
  }
...
TSpline3* Reconstruction::EnergyAfter2EnergyLoss(double emax, double size){
  double* eloss = new double[(int)(emax/size)+1];
  ...
  for(int i=0;i<(emax/size);i++){
    ...
    if(deloss < denergy){
      eloss[ctr]=deloss;
      after[ctr]=denergy-deloss;
      ctr++;
    }
  }
  TGraph* graph = new TGraph(ctr, after, eloss);
  TSpline3* spline = new TSpline3("EnergyAfter2EnergyLoss",graph);
  delete graph;
  delete[] eloss;
  delete[] after;
  return spline;
...
TSpline3* Reconstruction::Energy2EnergyAfter(double emax, double size){
  ...
  for(int i=0;i<(emax/size);i++){
    energy[i] = i*size;
    eloss[i] = EnergyLoss(energy[i],-5);//*1000.; //conversion to keV
    //cout << energy[i] <<"\t" << eloss[i]<<"\t" ;
    energy[i]=energy[i]-eloss[i]; 
    //cout << energy[i] << endl;
  ...
  TGraph* graph = new TGraph(through+1, eafter, eloss);
